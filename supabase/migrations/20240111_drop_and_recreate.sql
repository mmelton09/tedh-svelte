-- Force drop all variants of the functions
DROP FUNCTION IF EXISTS public.get_commander_pilots;
DROP FUNCTION IF EXISTS public.get_commander_pilot_tournaments;

-- RPC function to get aggregated pilot stats for a commander
CREATE FUNCTION get_commander_pilots(
  ...
  RETURN QUERY EXECUTE format('
    WITH commander_entries AS (
      ...
      GROUP BY te.player_id, te.entry_id, te.wins, te.losses, te.draws,
               te.standing, t.top_cut, t.total_players
    ),
    filtered_entries AS (
    ...
    SELECT
      ...
      ROUND((ps.conversions - ps.expected_conv) / NULLIF(ps.entries, 0) * 100, 1)::numeric,
      ROUND((ps.top4s - ps.expected_top4) / NULLIF(ps.entries, 0) * 100, 1)::numeric,
      ROUND((ps.championships - ps.expected_champ) / NULLIF(ps.entries, 0) * 100, 1)::numeric,
...
-- Tournament results function
CREATE FUNCTION get_commander_pilot_tournaments(
  p_commander_name text,
  p_start_date date DEFAULT '2020-01-01',
  p_end_date date DEFAULT CURRENT_DATE,
  p_min_size int DEFAULT 16,
